            ...
            <TableBody>
              {Facility?.data?.facility?.map((item: any) => (
                <TableRow key={item?._id}>
                  ...
                  <TableCell>
                    <button className={` font-semibold  px-3 py-1 rounded-md ${item?.isDeleted ? "disabled bg-gray-200" : "bg-red-100 text-red-500"}`} onClick={() => handelDelete(item?._id,user?.token as string)}>
